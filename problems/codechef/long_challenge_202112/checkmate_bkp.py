...
def is_in_check(xk, yk, x1, y1, x2, y2):
    if xk == x1 and yk == y1:
        if xk == x2 or yk == y2:
            return True
        else:
            return False
    elif xk == x2 and yk == y2:
        if xk == x1 or yk == y1:
            return True
        else:
            return False
    elif xk == x1 or xk == x2 or yk == y1 or yk == y2:
        return True
    else:
        return False
        ...
        for y_diff in [-1, 0, 1]:
            if x_diff != 0 or y_diff != 0:
                ...
                if 1 <= x_new <= 8 and 1 <= y_new <= 8:
                    if not is_in_check(x_new, y_new, x1, y1, x2, y2):
                        move_found = True
...
def can_check_mate(xk, yk, x1, y1, x2, y2):
    ...
    for x1_new, y1_new in get_next_rook_positions(x1, y1, x2, y2):
        ...
        if x1_new != x2 or y1_new != y2:
            if is_in_check(xk, yk, x1_new, y1_new, x2, y2) and not move_possible(xk, yk, x1_new, y1_new, x2, y2):
                return True
    ...
    for x2_new, y2_new in get_next_rook_positions(x2, y2, x1, y1):
        ...
        if x2_new != x2 or y2_new != y2:
            if is_in_check(xk, yk, x1, y1, x2_new, y2_new) and not move_possible(xk, yk, x1, y1, x2_new, y2_new):
                return True
